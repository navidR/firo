...
BOOST_AUTO_TEST_CASE(wallet_cover_set_metadata_skeleton_requires_accepted_stake_first)
{
    BOOST_CHECK(helsing::CheckHelsingWalletCoverSetMetadataSkeleton(false, false, false, false) == helsing::HelsingWalletCoverSetMetadataSkeletonResult::ACCEPTED_STAKE_UNAVAILABLE);
    BOOST_CHECK(helsing::CheckHelsingWalletCoverSetMetadataSkeleton(false, true, true, true) == helsing::HelsingWalletCoverSetMetadataSkeletonResult::ACCEPTED_STAKE_UNAVAILABLE);
}

BOOST_AUTO_TEST_CASE(wallet_cover_set_metadata_skeleton_orders_metadata_blockers)
{
    BOOST_CHECK(helsing::CheckHelsingWalletCoverSetMetadataSkeleton(true, false, true, true) == helsing::HelsingWalletCoverSetMetadataSkeletonResult::STAKING_COVER_SET_METADATA_PERSISTENCE_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckHelsingWalletCoverSetMetadataSkeleton(true, false, false, false) == helsing::HelsingWalletCoverSetMetadataSkeletonResult::STAKING_COVER_SET_METADATA_PERSISTENCE_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckHelsingWalletCoverSetMetadataSkeleton(true, true, false, true) == helsing::HelsingWalletCoverSetMetadataSkeletonResult::METADATA_STORAGE_PRIVACY_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckHelsingWalletCoverSetMetadataSkeleton(true, true, true, false) == helsing::HelsingWalletCoverSetMetadataSkeletonResult::LATER_SPEND_OVERLAP_POLICY_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(wallet_cover_set_metadata_skeleton_has_no_accepting_or_mutating_path)
{
    helsing::CHelsingState state;
    const helsing::StakeRecord record = ActiveRecord(201, DeterministicPoint(201));
    BOOST_CHECK(state.AddActiveStake(record));

    const helsing::HelsingWalletCoverSetMetadataSkeletonResult result = helsing::CheckHelsingWalletCoverSetMetadataSkeleton(true, true, true, true);

    BOOST_CHECK(result == helsing::HelsingWalletCoverSetMetadataSkeletonResult::WALLET_INTEGRATION_UNIMPLEMENTED);
    BOOST_CHECK_EQUAL(state.GetStakeRecordCount(), 1U);
    BOOST_CHECK_EQUAL(state.GetActiveTagCount(), 1U);
    BOOST_CHECK_EQUAL(state.GetSpentTagCount(), 0U);
    BOOST_CHECK(state.IsActiveTag(record.T));
...
BOOST_AUTO_TEST_CASE(stake_cover_set_output_rules_skeleton_rejects_mismatched_and_invalid_records)
{
    const std::vector<helsing::OutputId> inCoinIDs = {Output(1, 0), Output(1, 1), Output(2, 0), Output(3, 0)};
    helsing::ValidationStateView view;
    std::map<helsing::OutputId, bool> sparkRules;

    unsigned char pointTag = 94;
    for (const helsing::OutputId& output_id : inCoinIDs) {
        view.sparkOutputs.emplace(output_id, EligibleOutput(output_id, pointTag));
        sparkRules.emplace(output_id, true);
        pointTag += 3;
    ...
    view.sparkOutputs[inCoinIDs[1]].output_id = Output(99, 0);
    BOOST_CHECK(helsing::CheckStakeCoverSetOutputRulesSkeleton(inCoinIDs, view, sparkRules) == helsing::StakeValidationResult::OUTPUT_ID_MISMATCH);

    view.sparkOutputs[inCoinIDs[1]] = EligibleOutput(inCoinIDs[1], 97);
    view.sparkOutputs[inCoinIDs[2]].S = GroupElement();
    BOOST_CHECK(helsing::CheckStakeCoverSetOutputRulesSkeleton(inCoinIDs, view, sparkRules) == helsing::StakeValidationResult::INVALID_OUTPUT_RECORD);

    view.sparkOutputs[inCoinIDs[2]] = EligibleOutput(inCoinIDs[2], 100);
    view.sparkOutputs[inCoinIDs[3]].nHeight = -1;
    BOOST_CHECK(helsing::CheckStakeCoverSetOutputRulesSkeleton(inCoinIDs, view, sparkRules) == helsing::StakeValidationResult::INVALID_OUTPUT_RECORD);
}

BOOST_AUTO_TEST_CASE(stake_cover_set_spark_rules_blocker_preserves_current_output_precedence)
{
    const std::vector<helsing::OutputId> inCoinIDs = {Output(1, 0), Output(1, 1), Output(2, 0), Output(3, 0)};
    helsing::ValidationStateView view;

    helsing::StakeCoverSetSparkRulesBlockedSkeletonResult result = helsing::CheckStakeCoverSetSparkRulesBlockedSkeleton(inCoinIDs, view);
    BOOST_CHECK(result.output_result == helsing::StakeValidationResult::OUTPUT_NOT_FOUND);
    BOOST_CHECK(result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::STAKE_OUTPUT_PREFIX_FAILED);

    view.sparkOutputs.emplace(inCoinIDs[0], EligibleOutput(inCoinIDs[0], 95));
    view.sparkOutputs[inCoinIDs[0]].helsing_eligible = false;
    result = helsing::CheckStakeCoverSetSparkRulesBlockedSkeleton(inCoinIDs, view);
    BOOST_CHECK(result.output_result == helsing::StakeValidationResult::OUTPUT_NOT_ELIGIBLE);
    BOOST_CHECK(result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::STAKE_OUTPUT_PREFIX_FAILED);

    view.sparkOutputs[inCoinIDs[0]].helsing_eligible = true;
    result = helsing::CheckStakeCoverSetSparkRulesBlockedSkeleton(inCoinIDs, view);
    BOOST_CHECK(result.output_result == helsing::StakeValidationResult::OUTPUT_NOT_FOUND);
    BOOST_CHECK(result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::STAKE_OUTPUT_PREFIX_FAILED);

    view.sparkOutputs.emplace(inCoinIDs[1], EligibleOutput(inCoinIDs[1], 98));
    view.sparkOutputs[inCoinIDs[1]].helsing_eligible = false;
    result = helsing::CheckStakeCoverSetSparkRulesBlockedSkeleton(inCoinIDs, view);
    BOOST_CHECK(result.output_result == helsing::StakeValidationResult::OUTPUT_NOT_ELIGIBLE);
    BOOST_CHECK(result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::STAKE_OUTPUT_PREFIX_FAILED);
    ...
    view.sparkOutputs[inCoinIDs[1]].helsing_eligible = true;
    view.sparkOutputs.emplace(inCoinIDs[2], EligibleOutput(inCoinIDs[2], 101));
    view.sparkOutputs.emplace(inCoinIDs[3], EligibleOutput(inCoinIDs[3], 104));
    result = helsing::CheckStakeCoverSetSparkRulesBlockedSkeleton(inCoinIDs, view);
    BOOST_CHECK(result.output_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::SPARK_MATURITY_OR_COVER_SET_RULES_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(stake_cover_set_spark_rules_blocker_checks_record_identity_and_validity)
{
    const std::vector<helsing::OutputId> inCoinIDs = {Output(1, 0), Output(1, 1), Output(2, 0), Output(3, 0)};
    helsing::ValidationStateView view;

    unsigned char pointTag = 107;
    for (const helsing::OutputId& output_id : inCoinIDs) {
        view.sparkOutputs.emplace(output_id, EligibleOutput(output_id, pointTag));
        pointTag += 3;
    }

    view.sparkOutputs[inCoinIDs[2]].output_id = Output(99, 0);
    helsing::StakeCoverSetSparkRulesBlockedSkeletonResult result = helsing::CheckStakeCoverSetSparkRulesBlockedSkeleton(inCoinIDs, view);
    BOOST_CHECK(result.output_result == helsing::StakeValidationResult::OUTPUT_ID_MISMATCH);
    BOOST_CHECK(result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::STAKE_OUTPUT_PREFIX_FAILED);

    view.sparkOutputs[inCoinIDs[2]] = EligibleOutput(inCoinIDs[2], 110);
    view.sparkOutputs[inCoinIDs[3]].K = GroupElement();
    result = helsing::CheckStakeCoverSetSparkRulesBlockedSkeleton(inCoinIDs, view);
    BOOST_CHECK(result.output_result == helsing::StakeValidationResult::INVALID_OUTPUT_RECORD);
    BOOST_CHECK(result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::STAKE_OUTPUT_PREFIX_FAILED);
}

BOOST_AUTO_TEST_CASE(stake_cover_set_spark_rules_blocker_has_no_accepting_path)
{
    const std::vector<helsing::OutputId> inCoinIDs = {Output(1, 0), Output(1, 1), Output(2, 0), Output(3, 0)};
    helsing::ValidationStateView view;
    ...
    const helsing::StakeCoverSetSparkRulesBlockedSkeletonResult result = helsing::CheckStakeCoverSetSparkRulesBlockedSkeleton(inCoinIDs, view);
    BOOST_CHECK(result.output_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::SPARK_MATURITY_OR_COVER_SET_RULES_UNIMPLEMENTED);
}
...
BOOST_AUTO_TEST_CASE(stake_verification_outputs_skeleton_rejects_mismatched_and_invalid_records)
{
    helsing::StakeTx tx = ValidStakeTx();
    tx.inCoinIDs = {Output(1, 0), Output(1, 1), Output(2, 0), Output(3, 0)};
    ...
    helsing::StakeVerificationOutputSkeletonResult result = helsing::CheckStakeVerificationOutputsSkeleton(tx, state, view, 0, 1, true, true, true, true, true, true, 2, 2, sparkRules);
    BOOST_CHECK(result.cover_set_result.cover_set_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.output_result == helsing::StakeValidationResult::OUTPUT_ID_MISMATCH);

    view.sparkOutputs[tx.inCoinIDs[1]] = EligibleOutput(tx.inCoinIDs[1], 112);
    view.sparkOutputs[tx.inCoinIDs[2]].C = GroupElement();
    result = helsing::CheckStakeVerificationOutputsSkeleton(tx, state, view, 0, 1, true, true, true, true, true, true, 2, 2, sparkRules);
    BOOST_CHECK(result.cover_set_result.cover_set_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.output_result == helsing::StakeValidationResult::INVALID_OUTPUT_RECORD);
}
...
BOOST_AUTO_TEST_CASE(stake_proof_verification_skeleton_blocks_after_valid_prefix)
{
    ...
    BOOST_CHECK(helsing::CheckStakeProofVerificationSkeleton(prefix, false, false, false) == helsing::StakeProofVerificationSkeletonResult::STAKE_STATEMENT_UNAVAILABLE);
}

BOOST_AUTO_TEST_CASE(stake_proof_verification_skeleton_checks_proof_order)
{
    helsing::StakeVerificationOutputSkeletonResult prefix;
    prefix.cover_set_result.context_result.prefix_result = helsing::StakeVerificationPrefixSkeletonResult::OK;
    prefix.cover_set_result.context_result.tag_result = helsing::StakeValidationResult::OK;
    ...
    BOOST_CHECK(helsing::CheckStakeProofVerificationSkeleton(prefix, false, true, true) == helsing::StakeProofVerificationSkeletonResult::STAKE_STATEMENT_UNAVAILABLE);
    BOOST_CHECK(helsing::CheckStakeProofVerificationSkeleton(prefix, true, false, false) == helsing::StakeProofVerificationSkeletonResult::PAR_VERIFY_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeProofVerificationSkeleton(prefix, true, true, false) == helsing::StakeProofVerificationSkeletonResult::REP_VERIFY_UNIMPLEMENTED);
...
BOOST_AUTO_TEST_CASE(stake_statement_protocol_binding_skeleton_preserves_prefix_failures)
{
    helsing::StakeVerificationOutputSkeletonResult prefix;
    prefix.cover_set_result.context_result.prefix_result = helsing::StakeVerificationPrefixSkeletonResult::TX_INCOMPLETE;
    BOOST_CHECK(helsing::CheckStakeStatementProtocolBindingSkeleton(prefix, true, true, true, true) == helsing::StakeStatementProtocolBindingSkeletonResult::STAKE_PREFIX_FAILED);
    ...
    prefix.cover_set_result.cover_set_result = helsing::StakeValidationResult::OK;
    prefix.output_result = helsing::StakeValidationResult::OUTPUT_NOT_ELIGIBLE;
    BOOST_CHECK(helsing::CheckStakeStatementProtocolBindingSkeleton(prefix, true, true, true, true) == helsing::StakeStatementProtocolBindingSkeletonResult::STAKE_PREFIX_FAILED);
}

BOOST_AUTO_TEST_CASE(stake_statement_protocol_binding_skeleton_orders_public_binding_blockers)
{
    helsing::StakeVerificationOutputSkeletonResult prefix;
    prefix.cover_set_result.context_result.prefix_result = helsing::StakeVerificationPrefixSkeletonResult::OK;
    prefix.cover_set_result.context_result.tag_result = helsing::StakeValidationResult::OK;
    ...
    BOOST_CHECK(helsing::CheckStakeStatementProtocolBindingSkeleton(prefix, false, true, true, true) == helsing::StakeStatementProtocolBindingSkeletonResult::CHAIN_ID_BINDING_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeStatementProtocolBindingSkeleton(prefix, false, false, false, false) == helsing::StakeStatementProtocolBindingSkeletonResult::CHAIN_ID_BINDING_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeStatementProtocolBindingSkeleton(prefix, true, false, true, true) == helsing::StakeStatementProtocolBindingSkeletonResult::PROTOCOL_VERSION_BINDING_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeStatementProtocolBindingSkeleton(prefix, true, true, false, true) == helsing::StakeStatementProtocolBindingSkeletonResult::PUBLIC_PARAMETERS_HASH_BINDING_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeStatementProtocolBindingSkeleton(prefix, true, true, true, false) == helsing::StakeStatementProtocolBindingSkeletonResult::COLLATERAL_VALUE_BINDING_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(stake_statement_protocol_binding_skeleton_has_no_accepting_or_mutating_path)
{
    helsing::CHelsingState state;
    const helsing::StakeRecord record = ActiveRecord(205, DeterministicPoint(205));
    BOOST_CHECK(state.AddActiveStake(record));
    ...
    const helsing::StakeStatementProtocolBindingSkeletonResult result = helsing::CheckStakeStatementProtocolBindingSkeleton(prefix, true, true, true, true);

    BOOST_CHECK(result == helsing::StakeStatementProtocolBindingSkeletonResult::CONSENSUS_WIRING_UNIMPLEMENTED);
    BOOST_CHECK_EQUAL(state.GetStakeRecordCount(), 1U);
    BOOST_CHECK_EQUAL(state.GetActiveTagCount(), 1U);
    BOOST_CHECK_EQUAL(state.GetSpentTagCount(), 0U);
    BOOST_CHECK(state.IsActiveTag(record.T));
...
BOOST_AUTO_TEST_CASE(stake_proof_verification_skeleton_preserves_prefix_failures)
{
    helsing::StakeVerificationOutputSkeletonResult prefix;
    prefix.cover_set_result.context_result.prefix_result = helsing::StakeVerificationPrefixSkeletonResult::TX_INCOMPLETE;
    BOOST_CHECK(helsing::CheckStakeProofVerificationSkeleton(prefix, true, true, true) == helsing::StakeProofVerificationSkeletonResult::STAKE_PREFIX_FAILED);
...
BOOST_AUTO_TEST_CASE(stake_verification_blocked_skeleton_reports_spark_rule_blocker_after_cover_set_prefix)
{
    ...
    const helsing::StakeVerificationBlockedSkeletonResult result = helsing::CheckStakeVerificationBlockedSkeleton(tx, state, view, 0, 1, true, true, true, true, true, true, 2, 2, sparkRules);

    BOOST_CHECK(result.output_result.cover_set_result.context_result.prefix_result == helsing::StakeVerificationPrefixSkeletonResult::OK);
    BOOST_CHECK(result.output_result.cover_set_result.context_result.tag_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.output_result.cover_set_result.context_result.context_valid);
    BOOST_CHECK(result.output_result.cover_set_result.cover_set_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.output_result.output_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.spark_rules_result.output_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.spark_rules_result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::SPARK_MATURITY_OR_COVER_SET_RULES_UNIMPLEMENTED);
    BOOST_CHECK(result.statement_result == helsing::StakeStatementConstructionSkeletonResult::STAKE_PREFIX_FAILED);
    BOOST_CHECK(result.proof_result == helsing::StakeProofVerificationSkeletonResult::STAKE_PREFIX_FAILED);
}

BOOST_AUTO_TEST_CASE(stake_verification_blocked_skeleton_does_not_trust_caller_spark_rule_facts)
{
    helsing::StakeTx tx = ValidStakeTx();
    tx.inCoinIDs = {Output(1, 0), Output(1, 1), Output(2, 0), Output(3, 0)};
    ...
    unsigned char pointTag = 118;
    for (const helsing::OutputId& output_id : tx.inCoinIDs) {
        view.sparkOutputs.emplace(output_id, EligibleOutput(output_id, pointTag));
        sparkRules.emplace(output_id, false);
        pointTag += 3;
    }
...
BOOST_AUTO_TEST_CASE(stake_verification_blocked_skeleton_checks_prefix_before_proofs)
{
    helsing::StakeTx tx = ValidStakeTx();
    ...
    const helsing::StakeVerificationBlockedSkeletonResult result = helsing::CheckStakeVerificationBlockedSkeleton(tx, state, view, -1, 0, false, false, false, false, false, false, 2, 2, sparkRules);

    BOOST_CHECK(result.output_result.cover_set_result.context_result.prefix_result == helsing::StakeVerificationPrefixSkeletonResult::TX_INCOMPLETE);
    BOOST_CHECK(result.output_result.cover_set_result.context_result.tag_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.output_result.output_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.spark_rules_result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::STAKE_OUTPUT_PREFIX_FAILED);
    BOOST_CHECK(result.statement_result == helsing::StakeStatementConstructionSkeletonResult::STAKE_PREFIX_FAILED);
    BOOST_CHECK(result.proof_result == helsing::StakeProofVerificationSkeletonResult::STAKE_PREFIX_FAILED);
}

BOOST_AUTO_TEST_CASE(stake_verification_blocked_skeleton_checks_output_rules_before_proofs)
{
    helsing::StakeTx tx = ValidStakeTx();
    tx.inCoinIDs = {Output(1, 0), Output(1, 1), Output(2, 0), Output(3, 0)};
    ...
    BOOST_CHECK(result.output_result.cover_set_result.context_result.prefix_result == helsing::StakeVerificationPrefixSkeletonResult::OK);
    BOOST_CHECK(result.output_result.cover_set_result.cover_set_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.output_result.output_result == helsing::StakeValidationResult::OUTPUT_NOT_FOUND);
    BOOST_CHECK(result.spark_rules_result.output_result == helsing::StakeValidationResult::OUTPUT_NOT_FOUND);
    BOOST_CHECK(result.spark_rules_result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::STAKE_OUTPUT_PREFIX_FAILED);
    BOOST_CHECK(result.statement_result == helsing::StakeStatementConstructionSkeletonResult::STAKE_PREFIX_FAILED);
    BOOST_CHECK(result.proof_result == helsing::StakeProofVerificationSkeletonResult::STAKE_PREFIX_FAILED);
}

BOOST_AUTO_TEST_CASE(stake_verification_blocked_skeleton_checks_later_output_failures_before_spark_rules)
{
    helsing::StakeTx tx = ValidStakeTx();
    tx.inCoinIDs = {Output(1, 0), Output(1, 1), Output(2, 0), Output(3, 0)};
    ...
    view.sparkOutputs.emplace(tx.inCoinIDs[0], EligibleOutput(tx.inCoinIDs[0], 121));
    sparkRules.emplace(tx.inCoinIDs[0], true);
    helsing::StakeVerificationBlockedSkeletonResult result = helsing::CheckStakeVerificationBlockedSkeleton(tx, state, view, 0, 1, true, true, true, true, true, true, 2, 2, sparkRules);
    BOOST_CHECK(result.output_result.cover_set_result.cover_set_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.spark_rules_result.output_result == helsing::StakeValidationResult::OUTPUT_NOT_FOUND);
    BOOST_CHECK(result.spark_rules_result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::STAKE_OUTPUT_PREFIX_FAILED);

    view.sparkOutputs.emplace(tx.inCoinIDs[1], EligibleOutput(tx.inCoinIDs[1], 124));
    view.sparkOutputs[tx.inCoinIDs[1]].helsing_eligible = false;
    sparkRules.emplace(tx.inCoinIDs[1], true);
    result = helsing::CheckStakeVerificationBlockedSkeleton(tx, state, view, 0, 1, true, true, true, true, true, true, 2, 2, sparkRules);
    BOOST_CHECK(result.output_result.cover_set_result.cover_set_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.spark_rules_result.output_result == helsing::StakeValidationResult::OUTPUT_NOT_ELIGIBLE);
    BOOST_CHECK(result.spark_rules_result.spark_rules_result == helsing::StakeCoverSetSparkRulesSkeletonResult::STAKE_OUTPUT_PREFIX_FAILED);
}

BOOST_AUTO_TEST_CASE(stake_verification_blocked_skeleton_does_not_mutate_state)
{
    helsing::StakeTx tx = ValidStakeTx();
    tx.inCoinIDs = {Output(1, 0), Output(1, 1), Output(2, 0), Output(3, 0)};
...
BOOST_AUTO_TEST_CASE(payout_public_field_skeleton_accepts_caller_supplied_expected_values)
{
    ...
    BOOST_CHECK(helsing::CheckPayoutPublicFieldsSkeleton(tx, context, registeredAddress, tx.selected_stake_id, tx.V_PAYOUT, tx.V_PAYOUT + 1, true, expectedCoin) == helsing::PayoutPublicValidationResult::OK);
}
...
BOOST_AUTO_TEST_CASE(payout_public_field_skeleton_reports_each_ordered_failure)
{
    ...
    BOOST_CHECK(helsing::CheckPayoutPublicFieldsSkeleton(tx, context, wrongAddress, tx.selected_stake_id, tx.V_PAYOUT, tx.V_PAYOUT + 1, true, expectedCoin) == helsing::PayoutPublicValidationResult::ADDRESS_MISMATCH);

    BOOST_CHECK(helsing::CheckPayoutPublicFieldsSkeleton(tx, context, registeredAddress, DeterministicHash(44), tx.V_PAYOUT, tx.V_PAYOUT + 1, true, expectedCoin) == helsing::PayoutPublicValidationResult::SELECTED_STAKE_MISMATCH);
    BOOST_CHECK(helsing::CheckPayoutPublicFieldsSkeleton(tx, context, registeredAddress, tx.selected_stake_id, tx.V_PAYOUT + 1, tx.V_PAYOUT + 2, true, expectedCoin) == helsing::PayoutPublicValidationResult::INVALID_PAYOUT_AMOUNT);
    BOOST_CHECK(helsing::CheckPayoutPublicFieldsSkeleton(tx, context, registeredAddress, tx.selected_stake_id, tx.V_PAYOUT, tx.V_PAYOUT + 1, false, expectedCoin) == helsing::PayoutPublicValidationResult::INVALID_PAYOUT_AMOUNT);

    helsing::PayoutBlockContextSkeleton wrongContext = context;
    ++wrongContext.payout_index;
    BOOST_CHECK(helsing::CheckPayoutPublicFieldsSkeleton(tx, wrongContext, registeredAddress, tx.selected_stake_id, tx.V_PAYOUT, tx.V_PAYOUT + 1, true, expectedCoin) == helsing::PayoutPublicValidationResult::INVALID_PAYOUT_ID_INPUTS);

    helsing::PayoutCoinBlob wrongCoin = expectedCoin;
    wrongCoin.bytes = {0x64};
    BOOST_CHECK(helsing::CheckPayoutPublicFieldsSkeleton(tx, context, registeredAddress, tx.selected_stake_id, tx.V_PAYOUT, tx.V_PAYOUT + 1, true, wrongCoin) == helsing::PayoutPublicValidationResult::PAYOUT_COIN_MISMATCH);
}
...
BOOST_AUTO_TEST_CASE(payout_public_field_skeleton_preserves_section_eighteen_precedence)
{
    ...
    BOOST_CHECK(helsing::CheckPayoutPublicFieldsSkeleton(tx, wrongContext, wrongAddress, DeterministicHash(47), tx.V_PAYOUT + 1, tx.V_PAYOUT, false, wrongCoin) == helsing::PayoutPublicValidationResult::ADDRESS_MISMATCH);
    BOOST_CHECK(helsing::CheckPayoutPublicFieldsSkeleton(tx, wrongContext, registeredAddress, DeterministicHash(47), tx.V_PAYOUT + 1, tx.V_PAYOUT, false, wrongCoin) == helsing::PayoutPublicValidationResult::SELECTED_STAKE_MISMATCH);
    BOOST_CHECK(helsing::CheckPayoutPublicFieldsSkeleton(tx, wrongContext, registeredAddress, tx.selected_stake_id, tx.V_PAYOUT + 1, tx.V_PAYOUT, false, wrongCoin) == helsing::PayoutPublicValidationResult::INVALID_PAYOUT_AMOUNT);
    BOOST_CHECK(helsing::CheckPayoutPublicFieldsSkeleton(tx, wrongContext, registeredAddress, tx.selected_stake_id, tx.V_PAYOUT, tx.V_PAYOUT + 1, true, wrongCoin) == helsing::PayoutPublicValidationResult::INVALID_PAYOUT_ID_INPUTS);
}
...
BOOST_AUTO_TEST_CASE(validation_precedence_spent_then_block_then_active_then_outputs)
{
    helsing::StakeTx tx = ValidStakeTx();
    helsing::ValidationStateView view = ValidView(tx);
    helsing::CHelsingState state;
    ...
    view = ValidView(tx);
    view.helsingState = &state;
    BOOST_CHECK(state.AddActiveStake(ActiveRecord(89, tx.T)));
    view.sparkOutputs.clear();
    BOOST_CHECK(helsing::CheckStakeSkeleton(tx, view) == helsing::StakeValidationResult::TAG_ALREADY_ACTIVE);
...
BOOST_AUTO_TEST_CASE(block_spent_tag_builder_rejects_invalid_tags_without_mutation)
{
    const GroupElement tag = DeterministicPoint(142);
    const GroupElement sentinel = DeterministicPoint(143);

    std::unordered_set<GroupElement, spark::CLTagHash> blockSpentTags;
    blockSpentTags.insert(sentinel);

    BOOST_CHECK(helsing::BuildBlockSpentTagsSkeleton({tag, GroupElement()}, nullptr, blockSpentTags) == helsing::StakeValidationResult::INVALID_GROUP_ELEMENT);
    BOOST_CHECK_EQUAL(blockSpentTags.size(), 1U);
    BOOST_CHECK(blockSpentTags.count(sentinel) == 1);
    BOOST_CHECK(blockSpentTags.count(tag) == 0);

    BOOST_CHECK(helsing::BuildBlockSpentTagsSkeleton({NonMemberPoint()}, nullptr, blockSpentTags) == helsing::StakeValidationResult::INVALID_GROUP_ELEMENT);
    BOOST_CHECK_EQUAL(blockSpentTags.size(), 1U);
    BOOST_CHECK(blockSpentTags.count(sentinel) == 1);
}
...
BOOST_AUTO_TEST_CASE(payout_verification_skeleton_accepts_eligible_payout_with_expected_public_fields)
{
    ...
    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::PayoutVerificationSkeletonResult result = helsing::CheckPayoutVerificationSkeleton(payout, view, record.nHeight + 10, 10, context, registeredAddress, record.stake_id, payout.V_PAYOUT, payout.V_PAYOUT + 1, true, expectedCoin);
...
BOOST_AUTO_TEST_CASE(payout_verification_eligibility_skeleton_accepts_complete_active_mature_payout)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(59, DeterministicPoint(59));
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = record.stake_id;
    payout.payout_index = 30;
    payout.addr_pk.bytes = {0x61};
    payout.V_PAYOUT = 55;
    payout.coin.bytes = {0x63};

    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::PayoutVerificationEligibilitySkeletonResult result = helsing::CheckPayoutVerificationEligibilitySkeleton(payout, view, record.nHeight + 10, 10, true);

    BOOST_CHECK(result.prefix_result == helsing::PayoutVerificationPrefixSkeletonResult::OK);
    BOOST_CHECK(result.stake_result == helsing::StakeValidationResult::OK);
}

BOOST_AUTO_TEST_CASE(payout_verification_eligibility_skeleton_checks_prefix_before_state)
{
    helsing::ValidationStateView view;
    helsing::PayoutTxSkeleton incompletePayout;
    incompletePayout.selected_stake_id = DeterministicHash(60);
    incompletePayout.payout_index = 31;
    incompletePayout.addr_pk.bytes = {0x61};
    helsing::PayoutTxSkeleton noncanonicalPayout = incompletePayout;
    noncanonicalPayout.coin.bytes = {0x63};

    const helsing::PayoutVerificationEligibilitySkeletonResult incompleteResult = helsing::CheckPayoutVerificationEligibilitySkeleton(incompletePayout, view, 300, 0, false);
    const helsing::PayoutVerificationEligibilitySkeletonResult noncanonicalResult = helsing::CheckPayoutVerificationEligibilitySkeleton(noncanonicalPayout, view, 300, 0, false);

    BOOST_CHECK(incompleteResult.prefix_result == helsing::PayoutVerificationPrefixSkeletonResult::TX_INCOMPLETE);
    BOOST_CHECK(incompleteResult.stake_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(noncanonicalResult.prefix_result == helsing::PayoutVerificationPrefixSkeletonResult::MALFORMED_OR_NONCANONICAL);
    BOOST_CHECK(noncanonicalResult.stake_result == helsing::StakeValidationResult::OK);
}

BOOST_AUTO_TEST_CASE(payout_verification_eligibility_skeleton_reports_eligibility_after_prefix)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    ...
    BOOST_CHECK(state.AddActiveStake(record));
    view.blockSpentTags.insert(record.T);
    const helsing::PayoutVerificationEligibilitySkeletonResult result = helsing::CheckPayoutVerificationEligibilitySkeleton(payout, view, record.nHeight - 1, 10, true);

    BOOST_CHECK(result.prefix_result == helsing::PayoutVerificationPrefixSkeletonResult::OK);
    BOOST_CHECK(result.stake_result == helsing::StakeValidationResult::TAG_SPENT_IN_BLOCK);
}

BOOST_AUTO_TEST_CASE(payout_verification_eligibility_skeleton_does_not_mutate_state)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(62, DeterministicPoint(62));
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = record.stake_id;
    payout.payout_index = 33;
    payout.addr_pk.bytes = {0x61};
    payout.V_PAYOUT = 57;
    payout.coin.bytes = {0x63};
...
BOOST_AUTO_TEST_CASE(payout_verification_suffix_skeleton_blocks_after_eligibility)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(63, DeterministicPoint(63));
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = record.stake_id;
    payout.payout_index = 34;
    payout.addr_pk.bytes = {0x61};
    payout.V_PAYOUT = 58;
    payout.coin.bytes = {0x63};

    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::PayoutVerificationEligibilitySkeletonResult eligibility = helsing::CheckPayoutVerificationEligibilitySkeleton(payout, view, record.nHeight + 10, 10, true);

    BOOST_CHECK(eligibility.prefix_result == helsing::PayoutVerificationPrefixSkeletonResult::OK);
    BOOST_CHECK(eligibility.stake_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(helsing::CheckPayoutVerificationSuffixSkeleton(eligibility, true, true, true, true, true) == helsing::PayoutVerificationSuffixSkeletonResult::PAYOUT_COIN_COMPARISON_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(payout_verification_suffix_skeleton_checks_eligibility_before_suffix)
{
    helsing::ValidationStateView emptyView;
    helsing::PayoutTxSkeleton incompletePayout;
    incompletePayout.selected_stake_id = DeterministicHash(64);
    incompletePayout.payout_index = 35;
    incompletePayout.addr_pk.bytes = {0x61};

    const helsing::PayoutVerificationEligibilitySkeletonResult prefixFailure = helsing::CheckPayoutVerificationEligibilitySkeleton(incompletePayout, emptyView, 300, 0, false);

    BOOST_CHECK(prefixFailure.prefix_result == helsing::PayoutVerificationPrefixSkeletonResult::TX_INCOMPLETE);
    BOOST_CHECK(helsing::CheckPayoutVerificationSuffixSkeleton(prefixFailure, false, false, false, false, false) == helsing::PayoutVerificationSuffixSkeletonResult::PAYOUT_ELIGIBILITY_FAILED);

    helsing::PayoutTxSkeleton missingStakePayout;
    missingStakePayout.selected_stake_id = DeterministicHash(65);
    missingStakePayout.payout_index = 36;
    missingStakePayout.addr_pk.bytes = {0x61};
    missingStakePayout.V_PAYOUT = 59;
    missingStakePayout.coin.bytes = {0x63};
    const helsing::PayoutVerificationEligibilitySkeletonResult stakeFailure = helsing::CheckPayoutVerificationEligibilitySkeleton(missingStakePayout, emptyView, 300, 0, true);

    BOOST_CHECK(stakeFailure.prefix_result == helsing::PayoutVerificationPrefixSkeletonResult::OK);
    BOOST_CHECK(stakeFailure.stake_result == helsing::StakeValidationResult::STAKE_RECORD_NOT_FOUND);
    BOOST_CHECK(helsing::CheckPayoutVerificationSuffixSkeleton(stakeFailure, false, false, false, false, false) == helsing::PayoutVerificationSuffixSkeletonResult::PAYOUT_ELIGIBILITY_FAILED);
}

BOOST_AUTO_TEST_CASE(payout_verification_suffix_skeleton_orders_unimplemented_steps)
{
    helsing::PayoutVerificationEligibilitySkeletonResult eligibility;

    BOOST_CHECK(helsing::CheckPayoutVerificationSuffixSkeleton(eligibility, false, false, false, false, false) == helsing::PayoutVerificationSuffixSkeletonResult::REGISTERED_PAYOUT_ADDRESS_EXTRACTION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutVerificationSuffixSkeleton(eligibility, true, false, false, false, false) == helsing::PayoutVerificationSuffixSkeletonResult::DETERMINISTIC_SELECTION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutVerificationSuffixSkeleton(eligibility, true, true, false, false, false) == helsing::PayoutVerificationSuffixSkeletonResult::EXPECTED_PAYOUT_AMOUNT_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutVerificationSuffixSkeleton(eligibility, true, true, true, false, false) == helsing::PayoutVerificationSuffixSkeletonResult::PAYOUT_ID_CONSTRUCTION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutVerificationSuffixSkeleton(eligibility, true, true, true, true, false) == helsing::PayoutVerificationSuffixSkeletonResult::PAYOUT_ALGORITHM_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutVerificationSuffixSkeleton(eligibility, true, true, true, true, true) == helsing::PayoutVerificationSuffixSkeletonResult::PAYOUT_COIN_COMPARISON_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(payout_verification_suffix_skeleton_does_not_mutate_state)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(66, DeterministicPoint(66));
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = record.stake_id;
    payout.payout_index = 37;
    payout.addr_pk.bytes = {0x61};
    payout.V_PAYOUT = 60;
    payout.coin.bytes = {0x63};
...
BOOST_AUTO_TEST_CASE(payout_verification_blocked_skeleton_reports_suffix_after_eligibility)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(67, DeterministicPoint(67));
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = record.stake_id;
    payout.payout_index = 38;
    payout.addr_pk.bytes = {0x61};
    payout.V_PAYOUT = 61;
    payout.coin.bytes = {0x63};

    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::PayoutVerificationBlockedSkeletonResult result = helsing::CheckPayoutVerificationBlockedSkeleton(payout, view, record.nHeight + 10, 10, true, true, true, true, true, true);

    BOOST_CHECK(result.eligibility_result.prefix_result == helsing::PayoutVerificationPrefixSkeletonResult::OK);
    BOOST_CHECK(result.eligibility_result.stake_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.suffix_result == helsing::PayoutVerificationSuffixSkeletonResult::PAYOUT_COIN_COMPARISON_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(payout_verification_blocked_skeleton_checks_prefix_before_suffix)
{
    helsing::ValidationStateView view;
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = DeterministicHash(68);
    payout.payout_index = 39;
    payout.addr_pk.bytes = {0x61};

    const helsing::PayoutVerificationBlockedSkeletonResult result = helsing::CheckPayoutVerificationBlockedSkeleton(payout, view, 300, 0, false, false, false, false, false, false);

    BOOST_CHECK(result.eligibility_result.prefix_result == helsing::PayoutVerificationPrefixSkeletonResult::TX_INCOMPLETE);
    BOOST_CHECK(result.eligibility_result.stake_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.suffix_result == helsing::PayoutVerificationSuffixSkeletonResult::PAYOUT_ELIGIBILITY_FAILED);
}

BOOST_AUTO_TEST_CASE(payout_verification_blocked_skeleton_checks_state_before_suffix)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(69, DeterministicPoint(69));
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = record.stake_id;
    payout.payout_index = 40;
    payout.addr_pk.bytes = {0x61};
    payout.V_PAYOUT = 62;
    payout.coin.bytes = {0x63};
    ...
    BOOST_CHECK(state.AddActiveStake(record));
    view.blockSpentTags.insert(record.T);
    const helsing::PayoutVerificationBlockedSkeletonResult result = helsing::CheckPayoutVerificationBlockedSkeleton(payout, view, record.nHeight + 10, 10, true, false, false, false, false, false);

    BOOST_CHECK(result.eligibility_result.prefix_result == helsing::PayoutVerificationPrefixSkeletonResult::OK);
    BOOST_CHECK(result.eligibility_result.stake_result == helsing::StakeValidationResult::TAG_SPENT_IN_BLOCK);
    BOOST_CHECK(result.suffix_result == helsing::PayoutVerificationSuffixSkeletonResult::PAYOUT_ELIGIBILITY_FAILED);
}

BOOST_AUTO_TEST_CASE(payout_verification_blocked_skeleton_orders_suffix_after_eligibility)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(70, DeterministicPoint(70));
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = record.stake_id;
    payout.payout_index = 41;
    payout.addr_pk.bytes = {0x61};
    payout.V_PAYOUT = 63;
    payout.coin.bytes = {0x63};
    ...
    BOOST_CHECK(helsing::CheckPayoutVerificationBlockedSkeleton(payout, view, record.nHeight + 10, 10, true, false, false, false, false, false).suffix_result == helsing::PayoutVerificationSuffixSkeletonResult::REGISTERED_PAYOUT_ADDRESS_EXTRACTION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutVerificationBlockedSkeleton(payout, view, record.nHeight + 10, 10, true, true, false, false, false, false).suffix_result == helsing::PayoutVerificationSuffixSkeletonResult::DETERMINISTIC_SELECTION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutVerificationBlockedSkeleton(payout, view, record.nHeight + 10, 10, true, true, true, false, false, false).suffix_result == helsing::PayoutVerificationSuffixSkeletonResult::EXPECTED_PAYOUT_AMOUNT_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutVerificationBlockedSkeleton(payout, view, record.nHeight + 10, 10, true, true, true, true, false, false).suffix_result == helsing::PayoutVerificationSuffixSkeletonResult::PAYOUT_ID_CONSTRUCTION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutVerificationBlockedSkeleton(payout, view, record.nHeight + 10, 10, true, true, true, true, true, false).suffix_result == helsing::PayoutVerificationSuffixSkeletonResult::PAYOUT_ALGORITHM_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(payout_verification_blocked_skeleton_does_not_mutate_state)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(71, DeterministicPoint(71));
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = record.stake_id;
    payout.payout_index = 42;
    payout.addr_pk.bytes = {0x61};
    payout.V_PAYOUT = 64;
    payout.coin.bytes = {0x63};
...
BOOST_AUTO_TEST_CASE(payout_verification_skeleton_checks_eligibility_before_public_fields)
{
    ...
    BOOST_CHECK(state.AddActiveStake(record));
    view.blockSpentTags.insert(record.T);
    const helsing::PayoutVerificationSkeletonResult result = helsing::CheckPayoutVerificationSkeleton(payout, view, record.nHeight - 1, 10, wrongContext, wrongAddress, DeterministicHash(51), payout.V_PAYOUT + 1, payout.V_PAYOUT, false, wrongCoin);
...
BOOST_AUTO_TEST_CASE(payout_verification_skeleton_reports_public_failure_after_eligibility_passes)
{
    ...
    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::PayoutVerificationSkeletonResult result = helsing::CheckPayoutVerificationSkeleton(payout, view, record.nHeight + 10, 10, context, wrongAddress, record.stake_id, payout.V_PAYOUT, payout.V_PAYOUT + 1, false, expectedCoin);

    BOOST_CHECK(result.stake_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.public_result == helsing::PayoutPublicValidationResult::ADDRESS_MISMATCH);
}

BOOST_AUTO_TEST_CASE(payout_verification_skeleton_rejects_missing_payout_scalar_order_bound)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(121, DeterministicPoint(121));
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = record.stake_id;
    payout.payout_index = 43;
    payout.addr_pk.bytes = {0x61};
    payout.V_PAYOUT = 65;
    payout.coin.bytes = {0x63};
    ...
    helsing::PayoutBlockContextSkeleton context;
    context.chain_id = {0x66};
    context.block_height = 607;
    context.prev_block_hash = DeterministicHash(121);
    context.payout_index = payout.payout_index;
    context.selected_stake_id = payout.selected_stake_id;
    ...
    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::PayoutVerificationSkeletonResult result = helsing::CheckPayoutVerificationSkeleton(payout, view, record.nHeight + 10, 10, context, registeredAddress, record.stake_id, payout.V_PAYOUT, payout.V_PAYOUT + 1, false, expectedCoin);

    BOOST_CHECK(result.stake_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.public_result == helsing::PayoutPublicValidationResult::INVALID_PAYOUT_AMOUNT);
}
...
BOOST_AUTO_TEST_CASE(stake_update_authorization_skeleton_blocks_after_valid_prefix)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(56, DeterministicPoint(56));
    helsing::StakeUpdateTx tx;
    tx.stake_id = record.stake_id;
    tx.m_new.bytes = {0x6d, 0x36};
    tx.sig_update.bytes = {0x73, 0x36};

    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::StakeUpdateVerificationSkeletonResult prefix = helsing::CheckStakeUpdateVerificationSkeleton(tx, view);
    const helsing::StakeUpdateAuthorizationSkeletonResult result = helsing::CheckStakeUpdateAuthorizationSkeleton(prefix, true, true, true);

    BOOST_CHECK(prefix.tx_complete);
    BOOST_CHECK(prefix.stake_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result == helsing::StakeUpdateAuthorizationSkeletonResult::UPDATE_SIGNATURE_VERIFICATION_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(stake_update_authorization_skeleton_checks_prefix_before_authorization)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(57, DeterministicPoint(57));
    helsing::StakeUpdateTx incompleteTx;
    incompleteTx.stake_id = record.stake_id;
    incompleteTx.m_new.bytes = {0x6d, 0x37};
    helsing::StakeUpdateTx blockedTx;
    blockedTx.stake_id = record.stake_id;
    blockedTx.m_new.bytes = {0x6d, 0x38};
    blockedTx.sig_update.bytes = {0x73, 0x38};

    BOOST_CHECK(state.AddActiveStake(record));
    view.blockSpentTags.insert(record.T);
    const helsing::StakeUpdateVerificationSkeletonResult incompletePrefix = helsing::CheckStakeUpdateVerificationSkeleton(incompleteTx, view);
    const helsing::StakeUpdateVerificationSkeletonResult blockedPrefix = helsing::CheckStakeUpdateVerificationSkeleton(blockedTx, view);

    BOOST_CHECK(!incompletePrefix.tx_complete);
    BOOST_CHECK(blockedPrefix.stake_result == helsing::StakeValidationResult::TAG_SPENT_IN_BLOCK);
    BOOST_CHECK(helsing::CheckStakeUpdateAuthorizationSkeleton(incompletePrefix, true, true, true) == helsing::StakeUpdateAuthorizationSkeletonResult::STAKE_UPDATE_PREFIX_FAILED);
    BOOST_CHECK(helsing::CheckStakeUpdateAuthorizationSkeleton(blockedPrefix, true, true, true) == helsing::StakeUpdateAuthorizationSkeletonResult::STAKE_UPDATE_PREFIX_FAILED);
}

BOOST_AUTO_TEST_CASE(stake_update_authorization_skeleton_orders_authorization_blockers)
{
    const helsing::StakeUpdateVerificationSkeletonResult prefix{true, helsing::StakeValidationResult::OK};

    BOOST_CHECK(helsing::CheckStakeUpdateAuthorizationSkeleton(prefix, false, false, false) == helsing::StakeUpdateAuthorizationSkeletonResult::UPDATE_PUBLIC_KEY_EXTRACTION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateAuthorizationSkeleton(prefix, true, false, true) == helsing::StakeUpdateAuthorizationSkeletonResult::CANONICAL_UPDATE_CONTEXT_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateAuthorizationSkeleton(prefix, true, true, false) == helsing::StakeUpdateAuthorizationSkeletonResult::UPDATE_SIGNATURE_HASHING_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateAuthorizationSkeleton(prefix, true, true, true) == helsing::StakeUpdateAuthorizationSkeletonResult::UPDATE_SIGNATURE_VERIFICATION_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(stake_update_authorization_skeleton_does_not_mutate_state)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(58, DeterministicPoint(58));
    helsing::StakeUpdateTx tx;
    tx.stake_id = record.stake_id;
    tx.m_new.bytes = {0x6d, 0x39};
    tx.sig_update.bytes = {0x73, 0x39};
...
BOOST_AUTO_TEST_CASE(stake_update_signature_message_skeleton_checks_prefix_first)
{
    ...
    BOOST_CHECK(helsing::CheckStakeUpdateSignatureMessageSkeleton(incompletePrefix, false, false, false, false, false) == helsing::StakeUpdateSignatureMessageSkeletonResult::STAKE_UPDATE_PREFIX_FAILED);
    BOOST_CHECK(helsing::CheckStakeUpdateSignatureMessageSkeleton(blockedPrefix, false, false, false, false, false) == helsing::StakeUpdateSignatureMessageSkeletonResult::STAKE_UPDATE_PREFIX_FAILED);
}

BOOST_AUTO_TEST_CASE(stake_update_signature_message_skeleton_orders_message_blockers)
{
    const helsing::StakeUpdateVerificationSkeletonResult prefix{true, helsing::StakeValidationResult::OK};

    BOOST_CHECK(helsing::CheckStakeUpdateSignatureMessageSkeleton(prefix, false, false, false, false, false) == helsing::StakeUpdateSignatureMessageSkeletonResult::UPDATE_PUBLIC_KEY_EXTRACTION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateSignatureMessageSkeleton(prefix, true, false, false, false, false) == helsing::StakeUpdateSignatureMessageSkeletonResult::CANONICAL_UPDATE_CONTEXT_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateSignatureMessageSkeleton(prefix, true, true, false, false, false) == helsing::StakeUpdateSignatureMessageSkeletonResult::ENC_CONTEXT_HASHING_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateSignatureMessageSkeleton(prefix, true, true, true, false, false) == helsing::StakeUpdateSignatureMessageSkeletonResult::STAKE_ID_BINDING_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateSignatureMessageSkeleton(prefix, true, true, true, true, false) == helsing::StakeUpdateSignatureMessageSkeletonResult::UPDATE_MESSAGE_HASHING_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateSignatureMessageSkeleton(prefix, true, true, true, true, true) == helsing::StakeUpdateSignatureMessageSkeletonResult::CONSENSUS_WIRING_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(stake_update_signature_message_skeleton_has_no_accepting_or_mutating_path)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    ...
    helsing::StakeUpdateTx tx;
    tx.stake_id = record.stake_id;
    tx.m_new.bytes = {0x6d, 0x3a};
    tx.sig_update.bytes = {0x73, 0x3a};

    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::StakeUpdateVerificationSkeletonResult prefix = helsing::CheckStakeUpdateVerificationSkeleton(tx, view);
    const helsing::StakeUpdateSignatureMessageSkeletonResult result = helsing::CheckStakeUpdateSignatureMessageSkeleton(prefix, true, true, true, true, true);

    BOOST_CHECK(prefix.tx_complete);
    BOOST_CHECK(prefix.stake_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result == helsing::StakeUpdateSignatureMessageSkeletonResult::CONSENSUS_WIRING_UNIMPLEMENTED);
    BOOST_CHECK_EQUAL(state.GetStakeRecordCount(), 1U);
    BOOST_CHECK_EQUAL(state.GetActiveTagCount(), 1U);
    BOOST_CHECK_EQUAL(state.GetSpentTagCount(), 0U);
    BOOST_CHECK(state.IsActiveTag(record.T));
...
BOOST_AUTO_TEST_CASE(stake_update_new_context_validation_skeleton_checks_prefix_first)
{
    const helsing::StakeUpdateVerificationSkeletonResult incompletePrefix{false, helsing::StakeValidationResult::OK};
    const helsing::StakeUpdateVerificationSkeletonResult blockedPrefix{true, helsing::StakeValidationResult::TAG_SPENT_IN_BLOCK};

    BOOST_CHECK(helsing::CheckStakeUpdateNewContextValidationSkeleton(incompletePrefix, false, false, false, false, false) == helsing::StakeUpdateNewContextValidationSkeletonResult::STAKE_UPDATE_PREFIX_FAILED);
    BOOST_CHECK(helsing::CheckStakeUpdateNewContextValidationSkeleton(blockedPrefix, false, false, false, false, false) == helsing::StakeUpdateNewContextValidationSkeletonResult::STAKE_UPDATE_PREFIX_FAILED);
}

BOOST_AUTO_TEST_CASE(stake_update_new_context_validation_skeleton_orders_context_blockers)
{
    const helsing::StakeUpdateVerificationSkeletonResult prefix{true, helsing::StakeValidationResult::OK};

    BOOST_CHECK(helsing::CheckStakeUpdateNewContextValidationSkeleton(prefix, false, false, false, false, false) == helsing::StakeUpdateNewContextValidationSkeletonResult::CANONICAL_NEW_CONTEXT_ENCODING_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateNewContextValidationSkeleton(prefix, true, false, false, false, false) == helsing::StakeUpdateNewContextValidationSkeletonResult::NEW_CONTEXT_PAYOUT_ADDRESS_VALIDATION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateNewContextValidationSkeleton(prefix, true, true, false, false, false) == helsing::StakeUpdateNewContextValidationSkeletonResult::NEW_CONTEXT_UPDATE_PUBLIC_KEY_VALIDATION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateNewContextValidationSkeleton(prefix, true, true, true, false, false) == helsing::StakeUpdateNewContextValidationSkeletonResult::NEW_CONTEXT_NODE_SIGNING_KEY_VALIDATION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateNewContextValidationSkeleton(prefix, true, true, true, true, false) == helsing::StakeUpdateNewContextValidationSkeletonResult::NEW_CONTEXT_MASTERNODE_RULES_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateNewContextValidationSkeleton(prefix, true, true, true, true, true) == helsing::StakeUpdateNewContextValidationSkeletonResult::CONSENSUS_WIRING_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(stake_update_new_context_validation_skeleton_has_no_accepting_or_mutating_path)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    ...
    helsing::StakeUpdateTx tx;
    tx.stake_id = record.stake_id;
    tx.m_new.bytes = {0x6d, 0x3d};
    tx.sig_update.bytes = {0x73, 0x3d};

    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::StakeUpdateVerificationSkeletonResult prefix = helsing::CheckStakeUpdateVerificationSkeleton(tx, view);
    const helsing::StakeUpdateNewContextValidationSkeletonResult result = helsing::CheckStakeUpdateNewContextValidationSkeleton(prefix, true, true, true, true, true);

    BOOST_CHECK(prefix.tx_complete);
    BOOST_CHECK(prefix.stake_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result == helsing::StakeUpdateNewContextValidationSkeletonResult::CONSENSUS_WIRING_UNIMPLEMENTED);
    BOOST_CHECK_EQUAL(state.GetStakeRecordCount(), 1U);
    BOOST_CHECK_EQUAL(state.GetActiveTagCount(), 1U);
    BOOST_CHECK_EQUAL(state.GetSpentTagCount(), 0U);
    BOOST_CHECK(state.IsActiveTag(record.T));
...
BOOST_AUTO_TEST_CASE(stake_update_verification_blocked_skeleton_reports_authorization_after_prefix)
{
    ...
    helsing::StakeUpdateTx tx;
    tx.stake_id = record.stake_id;
    tx.m_new.bytes = {0x6d, 0x40};
    tx.sig_update.bytes = {0x73, 0x40};

    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::StakeUpdateVerificationBlockedSkeletonResult result = helsing::CheckStakeUpdateVerificationBlockedSkeleton(tx, view, true, true, true);

    BOOST_CHECK(result.prefix_result.tx_complete);
    BOOST_CHECK(result.prefix_result.stake_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.authorization_result == helsing::StakeUpdateAuthorizationSkeletonResult::UPDATE_SIGNATURE_VERIFICATION_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(stake_update_verification_blocked_skeleton_checks_fields_before_authorization)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(64, DeterministicPoint(64));
    helsing::StakeUpdateTx tx;
    tx.stake_id = record.stake_id;
    tx.m_new.bytes = {0x6d, 0x41};

    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::StakeUpdateVerificationBlockedSkeletonResult result = helsing::CheckStakeUpdateVerificationBlockedSkeleton(tx, view, false, false, false);

    BOOST_CHECK(!result.prefix_result.tx_complete);
    BOOST_CHECK(result.prefix_result.stake_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.authorization_result == helsing::StakeUpdateAuthorizationSkeletonResult::STAKE_UPDATE_PREFIX_FAILED);
}

BOOST_AUTO_TEST_CASE(stake_update_verification_blocked_skeleton_checks_state_before_authorization)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(65, DeterministicPoint(65));
    helsing::StakeUpdateTx tx;
    tx.stake_id = record.stake_id;
    tx.m_new.bytes = {0x6d, 0x42};
    tx.sig_update.bytes = {0x73, 0x42};
    ...
    BOOST_CHECK(result.prefix_result.tx_complete);
    BOOST_CHECK(result.prefix_result.stake_result == helsing::StakeValidationResult::TAG_SPENT_IN_BLOCK);
    BOOST_CHECK(result.authorization_result == helsing::StakeUpdateAuthorizationSkeletonResult::STAKE_UPDATE_PREFIX_FAILED);
}

BOOST_AUTO_TEST_CASE(stake_update_verification_blocked_skeleton_orders_authorization_after_prefix)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    ...
    helsing::StakeUpdateTx tx;
    tx.stake_id = record.stake_id;
    tx.m_new.bytes = {0x6d, 0x43};
    tx.sig_update.bytes = {0x73, 0x43};

    BOOST_CHECK(state.AddActiveStake(record));

    BOOST_CHECK(helsing::CheckStakeUpdateVerificationBlockedSkeleton(tx, view, false, false, false).authorization_result == helsing::StakeUpdateAuthorizationSkeletonResult::UPDATE_PUBLIC_KEY_EXTRACTION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateVerificationBlockedSkeleton(tx, view, true, false, false).authorization_result == helsing::StakeUpdateAuthorizationSkeletonResult::CANONICAL_UPDATE_CONTEXT_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateVerificationBlockedSkeleton(tx, view, true, true, false).authorization_result == helsing::StakeUpdateAuthorizationSkeletonResult::UPDATE_SIGNATURE_HASHING_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckStakeUpdateVerificationBlockedSkeleton(tx, view, true, true, true).authorization_result == helsing::StakeUpdateAuthorizationSkeletonResult::UPDATE_SIGNATURE_VERIFICATION_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(stake_update_verification_blocked_skeleton_does_not_mutate_state)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    ...
    helsing::StakeUpdateTx tx;
    tx.stake_id = record.stake_id;
    tx.m_new.bytes = {0x6d, 0x44};
    tx.sig_update.bytes = {0x73, 0x44};
...
BOOST_AUTO_TEST_CASE(block_validation_prefix_with_spent_tags_skeleton_accepts_valid_phase_inputs)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    helsing::StakeTx stake = ValidStakeTx();
    const helsing::StakeRecord updateRecord = ActiveRecord(179, DeterministicPoint(179));
    const helsing::StakeRecord payoutRecord = ActiveRecord(180, DeterministicPoint(180));
    helsing::StakeUpdateTx update;
    helsing::PayoutTxSkeleton payout;
    update.stake_id = updateRecord.stake_id;
    update.m_new.bytes = {0x75, 0x70, 0x64};
    update.sig_update.bytes = {0x73, 0x69, 0x67};
    payout.selected_stake_id = payoutRecord.stake_id;
    payout.payout_index = 1;
    payout.V_PAYOUT = 1;

    AddOutputs(view, stake, 45);
    BOOST_CHECK(state.AddActiveStake(updateRecord));
    BOOST_CHECK(state.AddActiveStake(payoutRecord));

    const helsing::BlockValidationPrefixWithSpentTagsSkeletonResult result = helsing::CheckBlockValidationPrefixWithSpentTagsSkeleton({}, {stake}, {update}, {payout}, view, payoutRecord.nHeight + 10, 10);

    BOOST_CHECK(result.block_spent_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.validation_result == helsing::StakeValidationResult::OK);
}

BOOST_AUTO_TEST_CASE(block_validation_prefix_with_spent_tags_skeleton_checks_block_spends_before_stakes)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    helsing::StakeTx stake = ValidStakeTx();
    const GroupElement spentTag = DeterministicPoint(181);
    std::vector<GroupElement> sparkSpendTags{spentTag, spentTag};

    view.blockSpentTags.insert(stake.T);
    const helsing::BlockValidationPrefixWithSpentTagsSkeletonResult result = helsing::CheckBlockValidationPrefixWithSpentTagsSkeleton(sparkSpendTags, {stake}, {}, {}, view, 300, 0);

    BOOST_CHECK(result.block_spent_result == helsing::StakeValidationResult::DUPLICATE_SPENT_TAG_IN_BLOCK);
    BOOST_CHECK(result.validation_result == helsing::StakeValidationResult::OK);
}

BOOST_AUTO_TEST_CASE(block_validation_prefix_with_spent_tags_skeleton_rejects_invalid_tags_before_stakes)
{
    helsing::ValidationStateView view;
    helsing::StakeTx stake = ValidStakeTx();
    AddOutputs(view, stake, 46);

    const helsing::BlockValidationPrefixWithSpentTagsSkeletonResult result = helsing::CheckBlockValidationPrefixWithSpentTagsSkeleton({stake.T, GroupElement()}, {stake}, {}, {}, view, 300, 0);

    BOOST_CHECK(result.block_spent_result == helsing::StakeValidationResult::INVALID_GROUP_ELEMENT);
    BOOST_CHECK(result.validation_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(!view.HasBlockSpentTag(stake.T));
}

BOOST_AUTO_TEST_CASE(block_validation_prefix_with_spent_tags_skeleton_makes_tags_visible_to_stakes)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    helsing::StakeTx stake = ValidStakeTx();
    AddOutputs(view, stake, 50);

    const helsing::BlockValidationPrefixWithSpentTagsSkeletonResult result = helsing::CheckBlockValidationPrefixWithSpentTagsSkeleton({stake.T}, {stake}, {}, {}, view, 300, 0);

    BOOST_CHECK(result.block_spent_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.validation_result == helsing::StakeValidationResult::TAG_SPENT_IN_BLOCK);
}

BOOST_AUTO_TEST_CASE(block_validation_prefix_with_spent_tags_skeleton_makes_tags_visible_to_updates)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(182, DeterministicPoint(182));
    helsing::StakeUpdateTx update;
    update.stake_id = record.stake_id;
    update.m_new.bytes = {0x75, 0x70, 0x64};
    update.sig_update.bytes = {0x73, 0x69, 0x67};
    ...
    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::BlockValidationPrefixWithSpentTagsSkeletonResult result = helsing::CheckBlockValidationPrefixWithSpentTagsSkeleton({record.T}, {}, {update}, {}, view, 300, 0);

    BOOST_CHECK(result.block_spent_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.validation_result == helsing::StakeValidationResult::TAG_SPENT_IN_BLOCK);
}

BOOST_AUTO_TEST_CASE(block_validation_prefix_with_spent_tags_skeleton_makes_tags_visible_to_payouts)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(183, DeterministicPoint(183));
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = record.stake_id;
    payout.payout_index = 2;
    payout.V_PAYOUT = 1;

    BOOST_CHECK(state.AddActiveStake(record));
    const helsing::BlockValidationPrefixWithSpentTagsSkeletonResult result = helsing::CheckBlockValidationPrefixWithSpentTagsSkeleton({record.T}, {}, {}, {payout}, view, record.nHeight + 10, 10);

    BOOST_CHECK(result.block_spent_result == helsing::StakeValidationResult::OK);
    BOOST_CHECK(result.validation_result == helsing::StakeValidationResult::TAG_SPENT_IN_BLOCK);
}

BOOST_AUTO_TEST_CASE(block_validation_prefix_with_spent_tags_skeleton_does_not_mutate_state_or_view)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    const helsing::StakeRecord record = ActiveRecord(184, DeterministicPoint(184));
    helsing::PayoutTxSkeleton payout;
    payout.selected_stake_id = record.stake_id;
...
BOOST_AUTO_TEST_CASE(block_validation_spark_prepass_skeleton_blocks_tag_extraction_after_spark_validation)
{
    BOOST_CHECK(helsing::CheckBlockValidationSparkPrepassSkeleton(false) == helsing::BlockValidationSparkPrepassSkeletonResult::ORDINARY_SPARK_SPEND_VALIDATION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckBlockValidationSparkPrepassSkeleton(true) == helsing::BlockValidationSparkPrepassSkeletonResult::SPARK_SPEND_TAG_EXTRACTION_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(block_validation_blocked_skeleton_reports_spark_prepass_before_prefix)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
    ...
    BOOST_CHECK(result.spark_prepass_result == helsing::BlockValidationSparkPrepassSkeletonResult::ORDINARY_SPARK_SPEND_VALIDATION_UNIMPLEMENTED);
...
BOOST_AUTO_TEST_CASE(block_validation_blocked_skeleton_checks_spark_prepass_before_block_spends)
{
    helsing::ValidationStateView view;
...
BOOST_AUTO_TEST_CASE(block_validation_blocked_skeleton_checks_spark_prepass_before_stake_update_payout_prefix)
{
    helsing::CHelsingState state;
    helsing::ValidationStateView view;
    view.helsingState = &state;
...
BOOST_AUTO_TEST_CASE(validation_result_strings_cover_all_values)
{
    ...
    BOOST_CHECK_EQUAL(helsing::StakeValidationResultToString(helsing::StakeValidationResult::OUTPUT_SPARK_RULES_FAILED), "OUTPUT_SPARK_RULES_FAILED");
...
BOOST_AUTO_TEST_CASE(payout_id_construction_skeleton_blocks_incomplete_inputs_before_encoding)
{
    helsing::PayoutTxSkeleton tx;
    tx.selected_stake_id = DeterministicHash(251);
    tx.payout_index = 19;
    ...
    helsing::PayoutBlockContextSkeleton context;
    context.chain_id = {0x66};
    context.block_height = 504;
    context.prev_block_hash = DeterministicHash(252);
    context.payout_index = tx.payout_index;
    context.selected_stake_id = tx.selected_stake_id;
    BOOST_REQUIRE(helsing::ArePayoutIdInputsCompleteSkeleton(tx, context));

    helsing::PayoutBlockContextSkeleton incompleteContext = context;
    incompleteContext.chain_id.clear();
    BOOST_CHECK(helsing::CheckPayoutIdConstructionSkeleton(tx, incompleteContext, true) == helsing::PayoutIdConstructionSkeletonResult::PAYOUT_ID_INPUTS_INCOMPLETE);

    helsing::PayoutBlockContextSkeleton mismatchedContext = context;
    ++mismatchedContext.payout_index;
    BOOST_CHECK(helsing::CheckPayoutIdConstructionSkeleton(tx, mismatchedContext, false) == helsing::PayoutIdConstructionSkeletonResult::PAYOUT_ID_INPUTS_INCOMPLETE);
}

BOOST_AUTO_TEST_CASE(payout_id_construction_skeleton_blocks_missing_canonical_encoding)
{
    helsing::PayoutTxSkeleton tx;
    tx.selected_stake_id = DeterministicHash(253);
    tx.payout_index = 20;

    helsing::PayoutBlockContextSkeleton context;
    context.chain_id = {0x66};
    context.block_height = 505;
    context.prev_block_hash = DeterministicHash(254);
    context.payout_index = tx.payout_index;
    context.selected_stake_id = tx.selected_stake_id;

    BOOST_CHECK(helsing::CheckPayoutIdConstructionSkeleton(tx, context, false) == helsing::PayoutIdConstructionSkeletonResult::CANONICAL_PAYOUT_ID_ENCODING_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(payout_id_construction_skeleton_does_not_construct_identifier)
{
    helsing::PayoutTxSkeleton tx;
    tx.selected_stake_id = DeterministicHash(255);
    tx.payout_index = 21;

    helsing::PayoutBlockContextSkeleton context;
    context.chain_id = {0x66};
    context.block_height = 506;
    context.prev_block_hash = DeterministicHash(250);
    context.payout_index = tx.payout_index;
    context.selected_stake_id = tx.selected_stake_id;
...
BOOST_AUTO_TEST_CASE(payout_registered_address_extraction_skeleton_checks_eligibility_first)
{
    ...
    const helsing::PayoutVerificationEligibilitySkeletonResult stakeFailure{helsing::PayoutVerificationPrefixSkeletonResult::OK, helsing::StakeValidationResult::STAKE_NOT_ACTIVE};

    BOOST_CHECK(helsing::CheckPayoutRegisteredAddressExtractionSkeleton(prefixFailure, true, true, true) == helsing::PayoutRegisteredAddressExtractionSkeletonResult::PAYOUT_ELIGIBILITY_FAILED);
    BOOST_CHECK(helsing::CheckPayoutRegisteredAddressExtractionSkeleton(stakeFailure, true, true, true) == helsing::PayoutRegisteredAddressExtractionSkeletonResult::PAYOUT_ELIGIBILITY_FAILED);
}

BOOST_AUTO_TEST_CASE(payout_registered_address_extraction_skeleton_orders_extraction_blockers)
{
    const helsing::PayoutVerificationEligibilitySkeletonResult eligibility{helsing::PayoutVerificationPrefixSkeletonResult::OK, helsing::StakeValidationResult::OK};

    BOOST_CHECK(helsing::CheckPayoutRegisteredAddressExtractionSkeleton(eligibility, false, false, false) == helsing::PayoutRegisteredAddressExtractionSkeletonResult::EFFECTIVE_CONTEXT_SELECTION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutRegisteredAddressExtractionSkeleton(eligibility, true, false, true) == helsing::PayoutRegisteredAddressExtractionSkeletonResult::PAYOUT_ADDRESS_GRAMMAR_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutRegisteredAddressExtractionSkeleton(eligibility, true, true, false) == helsing::PayoutRegisteredAddressExtractionSkeletonResult::REGISTERED_PAYOUT_ADDRESS_EXTRACTION_UNIMPLEMENTED);
    BOOST_CHECK(helsing::CheckPayoutRegisteredAddressExtractionSkeleton(eligibility, true, true, true) == helsing::PayoutRegisteredAddressExtractionSkeletonResult::CONSENSUS_WIRING_UNIMPLEMENTED);
}

BOOST_AUTO_TEST_CASE(payout_registered_address_extraction_skeleton_does_not_mutate_state)
{
    helsing::CHelsingState state;
    const helsing::StakeRecord record = ActiveRecord(73, DeterministicPoint(73));
    const helsing::PayoutVerificationEligibilitySkeletonResult eligibility{helsing::PayoutVerificationPrefixSkeletonResult::OK, helsing::StakeValidationResult::OK};
