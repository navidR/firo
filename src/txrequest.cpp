...
struct Announcement {
    ...
    Announcement(const GenTxid& gtxid, NodeId peer, bool preferred, std::chrono::microseconds reqtime, SequenceNumber sequence) : m_txhash(gtxid.GetHash()), m_time(reqtime), m_peer(peer), m_sequence(sequence), m_preferred(preferred),
...
private:
    ...
    //! Convert a CANDIDATE_DELAYED announcement into a CANDIDATE_READY, and select it if it is best.
    void PromoteCandidateReady(Iter<ByTxHash> it)
    {
        ...
        // ByTxHash is ordered by txhash, state, and (for READY entries) priority. The selected entry,
        // if any, immediately follows the best READY entry.
    ...
    //! Change the state of an announcement to something non-selected, selecting the next best if needed.
    ...
    //! Check whether this is the only non-completed announcement for its txhash.
    bool IsOnlyNonCompleted(Iter<ByTxHash> it)
    {
        assert(it != m_index.get<ByTxHash>().end());
        assert(it->m_state != State::COMPLETED);
        ...
        if (std::next(it) != m_index.get<ByTxHash>().end() && std::next(it)->m_txhash == it->m_txhash &&
            std::next(it)->m_state != State::COMPLETED) {
            return false;
        }
        return true;
    }

    /** Mark an announcement completed. Delete every announcement for its txhash if no viable one remains. */
    ...
    void SetTimePoint(std::chrono::microseconds now, std::vector<std::pair<NodeId, GenTxid> >* expired)
    {
        ...
        // Handle the unusual case where the clock moves backwards.
        while (!m_index.empty()) {
...
public:
    ...
    void DisconnectedPeer(NodeId peer)
    {
        ...
        auto it = index.lower_bound(ByPeerView{peer, false, uint256()});
        while (it != index.end() && it->m_peer == peer) {
            // Decide where to continue before MakeCompleted potentially moves or deletes entries.
    ...
    std::vector<GenTxid> GetRequestable(NodeId peer, std::chrono::microseconds now, std::vector<std::pair<NodeId, GenTxid> >* expired)
    {
        ...
        auto it_peer = m_index.get<ByPeer>().lower_bound(ByPeerView{peer, true, uint256()});
    ...
    void RequestedTx(NodeId peer, const uint256& txhash, std::chrono::microseconds expiry)
    {
        ...
        if (it == m_index.get<ByPeer>().end()) {
            // Permit explicit requests that did not immediately follow GetRequestable while preserving
            // the one-selected-announcement invariant.
...
void TxRequestTracker::ReceivedInv(NodeId peer, const GenTxid& gtxid, bool preferred, std::chrono::microseconds reqtime)
{
...
std::vector<GenTxid> TxRequestTracker::GetRequestable(NodeId peer, std::chrono::microseconds now, std::vector<std::pair<NodeId, GenTxid> >* expired)
{
